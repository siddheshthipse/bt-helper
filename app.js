            ...
            hierarchyLevels.forEach((level, levelIndex) => {
                ...
                if (!idMap.has(compositeKey)) {
                    ...
                    const node = {
                        ...
                        // Include metadata fields only for leaf nodes (L5), otherwise empty
                        ID: row.ID || '',
                        'Business Role': level === 'L5 - Process Step' ? (row['Business Role'] || '') : '',
                        'Fiori app UX recommendations': level === 'L5 - Process Step' ? (row['Fiori app UX recommendations'] || '') : '',
                        'Insights (Indicative)': level === 'L5 - Process Step' ? (row['Insights (Indicative)'] || '') : '',
                        'Business stakeholders': level === 'L5 - Process Step' ? (row['Business stakeholders'] || '') : '',
                        'Materiality': level === 'L5 - Process Step' ? (row['Materiality'] || 0) : 0,
                        'Description': level === 'L5 - Process Step' ? (row['Description'] || '') : ''
                    };
